...
Sheet列表:
  1. 日超8H: 三级部门、四级部门、超过人数、超过小时合计、未超过人数、总计人数
  2. 是否排班: 三级部门、四级部门、已排班数、未排班数、总计人数、排班率
  3. 是否排班正确: 三级部门、四级部门、正确数量、不正确数量、总计人数、正确率
  4. 缺卡: 三级部门、四级部门、缺卡数量、标准打卡数量、缺卡率
  5. hub排班是否正确: 三级部门、四级部门、五级部门、正确、不正确、总计、正确率

分组规则:
  - 四级部门非空: 每个四级一条明细行
  - 四级部门为空: 汇总为一条 "/" 明细行
  - 每个三级部门末尾: 强制插入汇总行
  - Sheet5 按五级部门分组，仅筛选 HUB=='hub' 的记录

样式: 汇总行绿色背景加粗；表头蓝色背景白色字体
"""
import sys, os
...
# 配置区（按需修改）
# ============================================================
WORKSPACE  = r"C:\Users\Administrator\WorkBuddy\{workspace}"  # 替换 {workspace}
INPUT_FILE = os.path.join(WORKSPACE, "指标计算后数据.xlsx")
OUTPUT_FILE = os.path.join(WORKSPACE, "透视分析.xlsx")
...
df['_缺卡次数_num']   = df['缺卡次数'].apply(to_num)
...
for dept3, grp in df.groupby('三级部门'):
    ...
    for d4, sub in grp_nonempty.groupby('四级部门'):
        ...
        miss   = round(sub['_缺卡次数_num'].sum(), 2)
        ...
        rows_list.append({
            ...
            '缺卡数量': miss, '标准打卡数量': std, '_is_summary': False,
        })
    ...
    if len(grp_empty) > 0:
        ...
        miss   = round(grp_empty['_缺卡次数_num'].sum(), 2)
    ...
    miss   = round(grp['_缺卡次数_num'].sum(), 2)
    ...
    rows_list.append({
        ...
        '未超过人数': cnt - over, '已排班数': shift, '未排班数': cnt - shift,
        '正确数量': corr, '不正确数量': incorr, '总计人数': corr + incorr,
        '缺卡数量': miss, '标准打卡数量': std, '_is_summary': True,
    })
...
def style_cell(cell, is_summary, num_fmt=None):
...
def write_sheet(ws, col_headers, rows_data, rate_cols=None):
...
write_sheet(ws1, [(1,'三级部门'),(2,'四级部门'),(3,'超过人数'),(4,'超过小时合计'),(5,'未超过人数'),(6,'总计人数')], pivot_df.to_dict('records'))

ws2 = wb.create_sheet("是否排班")
write_sheet(ws2, [(1,'三级部门'),(2,'四级部门'),(3,'已排班数'),(4,'未排班数'),(5,'总计人数'),(6,'排班率')], pivot_df.to_dict('records'), rate_cols=[(6,'已排班数','计数')])

ws3 = wb.create_sheet("是否排班正确")
write_sheet(ws3, [(1,'三级部门'),(2,'四级部门'),(3,'正确数量'),(4,'不正确数量'),(5,'总计人数'),(6,'正确率')], pivot_df.to_dict('records'), rate_cols=[(6,'正确数量','计数')])

ws4 = wb.create_sheet("缺卡")
write_sheet(ws4, [(1,'三级部门'),(2,'四级部门'),(3,'缺卡数量'),(4,'标准打卡数量'),(5,'缺卡率')], pivot_df.to_dict('records'), rate_cols=[(5,'缺卡数量','标准打卡数量')])
...
write_sheet(ws5, [(1,'三级部门'),(2,'四级部门'),(3,'五级部门'),(4,'正确'),(5,'不正确'),(6,'总计'),(7,'正确率')], hub_df.to_dict('records'), rate_cols=[(7,'正确','总计')])
